...
const TeamBalances: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      let ltData: LeaveTypeInfo[] = await apiClient.get('/leave-types');
      const types = ltData || [];
      setLeaveTypes(types);
      ...
      const [allBalances, allApproved, allPending] = await Promise.all([
        apiClient.get('/leave-requests/scope', { user_ids: memberIds.join(','), status: 'approved', start_date: minStart, end_date: maxEnd }).catch(() => []),
      ...
      const result: StaffRow[] = staff.map((s) => {
        ...
        const ua = (Array.isArray(allApproved) ? allApproved : []).filter((r: any) => r.user_id === s.id || r.user_id?._id === s.id);
        const up = (Array.isArray(allPending) ? allPending : []).filter((r: any) => r.user_id === s.id || r.user_id?._id === s.id);
  ...
  return (
    <Box>
      ...
      {rows.length === 0 ? (
        <Paper sx={{ p: 4, textAlign: 'center' }}><Typography color="text.secondary">{langPackLabel("txtNoTeamMembersFound") || "No team members found."}</Typography></Paper>
      ) : (
        <Paper>
          <TableContainer>
            <Table size="small">
