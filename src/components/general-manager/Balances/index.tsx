// src/components/general-manager/Balances/index.tsx — Department-scoped balances (batched queries)
...
const DepartmentBalances: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.department_id) fetchData();
  }, [user?.department_id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 1 }}>
        <Typography variant="h5" sx={{ color: 'primary.main', fontWeight: 600 }}>{langPackLabel("txtBalances") || "Balances"}</Typography>
      ...
      {rows.length === 0 ? (
        <Paper sx={{ p: 4, textAlign: 'center' }}><Typography color="text.secondary">{langPackLabel("txtNoTeamMembersFound") || "No department members found."}</Typography></Paper>
      ) : (
        <Paper>
          <TableContainer>
            <Table size="small">
...
export default DepartmentBalances;
