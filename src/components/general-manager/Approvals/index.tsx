// src/components/general-manager/Approvals/index.tsx — Department Manager Approvals
...
const DepartmentManagerApprovals: React.FC = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: err.message || (langPackLabel("txtFailedToLoadRequests") || 'Failed to load pending requests'), severity: 'error' });
    } finally {
      setLoading(false);
    }
  ...
  const fetchPastDecisions = async () => {
    ...
    try {
      ...
      const data: LeaveRequest[] = await apiClient.get('/leave-requests/scope', {
        status: 'approved,rejected',
        approved_by: currentUser.id,
      });
      setPastRequests(((data ?? []) as LeaveRequest[]).slice(0, 50));
    } catch (err: any) {
      setSnackbar({ open: true, message: err.message || (langPackLabel("txtFailedToLoadPastDecisions") || 'Failed to load past decisions'), severity: 'error' });
    } finally {
  ...
  const handleActionConfirm = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: actionType === 'approved' ? (langPackLabel("txtRequestApprovedSuccess") || 'Request approved successfully.') : (langPackLabel("txtRequestRejectedSuccess") || 'Request rejected successfully.'), severity: 'success' });
      setDialogOpen(false);
      setActionTarget(null);
      setComment('');
      fetchRequests();
    } catch (err: any) {
      const msg = err.message || err || (langPackLabel("txtSomethingWentWrong") || 'Failed to update request');
      const isStale = typeof msg === 'string' && (msg.toLowerCase().includes('no longer pending') || msg.toLowerCase().includes('not found') || msg.toLowerCase().includes('0 rows'));
      setSnackbar({ open: true, message: isStale ? (langPackLabel("txtRequestNoLongerPending") || 'This request is no longer pending. The list has been refreshed.') : msg, severity: 'error' });
      setDialogOpen(false);
      setActionTarget(null);
      setComment('');
      fetchRequests();
  ...
  const isSelfRequest = (request: LeaveRequest) => currentUser ? request.user_id === currentUser.id : false;
  const getBalance = (request: LeaveRequest): LeaveBalanceSummary | undefined => balances[`${request.user_id}_${request.leave_type_id}`];

  return (
    <Box>
      <Typography variant="h5" sx={{ mb: 3, color: 'primary.main', fontWeight: 600 }}>{langPackLabel("txtPendingApprovals") || "Pending Approvals"}</Typography>

      <Paper>
        {loading ? (
          <Box display="flex" justifyContent="center" alignItems="center" minHeight="200px"><CircularProgress /></Box>
        ) : requests.length === 0 ? (
          <Box sx={{ p: 4, textAlign: 'center' }}><Typography variant="body1" color="text.secondary">{langPackLabel("txtNoPendingRequests") || "No pending leave requests to review."}</Typography></Box>
        ) : (
          <>
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>{langPackLabel("txtRemainingBalance") || "Remaining Balance"}</TableCell>
                  ...
                  {requests.map((req) => {
                    ...
                    return (
                      <TableRow key={req.id} hover>
                        ...
                        <TableCell>{formatLocalDate(req.start_date, language)} – {formatLocalDate(req.end_date, language)}</TableCell>
                        <TableCell>
                          {req.leave_type ? (
                            <Chip label={req.leave_type.name} size="small" sx={{ backgroundColor: req.leave_type.color_code, color: '#fff' }} />
                          ) : 'N/A'}
                        </TableCell>
                        ...
                        <TableCell>
                          {bal ? (
                            <Chip label={`${bal.remaining} days`} size="small" color={bal.remaining < 0 ? 'error' : bal.remaining <= req.total_days ? 'warning' : 'success'} variant="outlined" />
                          ) : '—'}
                        </TableCell>
                        <TableCell>
                          {self ? (
                            <Typography variant="caption" color="text.secondary">{langPackLabel("txtOwnRequest") || "Own request"}</Typography>
                          ) : (
                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                              <Button size="small" variant="contained" color="success" startIcon={<ApproveIcon />} onClick={() => handleActionClick(req, 'approved')}>{langPackLabel("txtApprove") || "Approve"}</Button>
                              <Button size="small" variant="outlined" color="error" startIcon={<RejectIcon />} onClick={() => handleActionClick(req, 'rejected')}>{langPackLabel("txtReject") || "Reject"}</Button>
                            </Box>
                          )}
                        </TableCell>
                      </TableRow>
      ...
      <Dialog open={dialogOpen} onClose={handleDialogClose} maxWidth="sm" fullWidth>
        <DialogTitle>{actionType === 'approved' ? (langPackLabel("txtApproveLeaveRequest") || 'Approve Leave Request') : (langPackLabel("txtRejectLeaveRequest") || 'Reject Leave Request')}</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            {actionTarget && (<>{actionType === 'approved' ? (langPackLabel("txtApproveConfirmText") || 'Approve the leave request from') : (langPackLabel("txtRejectConfirmText") || 'Reject the leave request from')} <strong>{actionTarget.user?.full_name ?? 'this employee'}</strong> {langPackLabel("txtFor") || "for"} <strong>{formatLocalDate(actionTarget.start_date, language)} – {formatLocalDate(actionTarget.end_date, language)}</strong> ({actionTarget.total_days} {langPackLabel("txtDay") || "Day"})?</>)}
          </DialogContentText>
          <TextField label={langPackLabel("txtCommentOptional") || "Comment (optional)"} multiline rows={3} fullWidth value={comment} onChange={(e) => setComment(e.target.value)} placeholder={langPackLabel("txtAddComment") || "Add a comment for the employee..."} />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleDialogClose} disabled={processing}>{langPackLabel("txtCancel") || "Cancel"}</Button>
          <Button onClick={handleActionConfirm} variant="contained" color={actionType === 'approved' ? 'success' : 'error'} disabled={processing} startIcon={processing ? <CircularProgress size={16} /> : undefined}>
            {processing ? (langPackLabel("txtProcessing") || 'Processing...') : actionType === 'approved' ? (langPackLabel("txtApprove") || 'Approve') : (langPackLabel("txtReject") || 'Reject')}
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <Collapse in={showPast}>
          ...
          <Paper>
            {pastLoading ? (
              ...
              <Box sx={{ p: 4, textAlign: 'center' }}><Typography color="text.secondary">{langPackLabel("txtNoPastDecisions") || "No past decisions found."}</Typography></Box>
            ) : (() => {
