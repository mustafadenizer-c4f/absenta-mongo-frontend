...
const Login: React.FC = () => {
  ...
  const { setLoginLanguagePack, langPackLabel } = useLanguage();
  ...
  const onSubmit: SubmitHandler<LoginFormInputs> = async (data) => {
    ...
    if (login.fulfilled.match(result)) {
      const payload = result.payload as { user: any; session: any; language_pack?: Record<string, string> };
      if (payload.language_pack) {
        setLoginLanguagePack(payload.language_pack, payload.user?.language || 'en');
      }
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box component="form" onSubmit={handleSubmit(onSubmit)} onInput={clearLoginError} noValidate>
              ...
              <Button
                ...
                disabled={submitting}
              >
                {submitting ? (langPackLabel("txtSigningIn") || 'Signing in...') : (langPackLabel("txtSignIn") || 'Sign In')}
              </Button>
