...
const AdminTeamCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const usersData: User[] = await apiClient.get('/users');
        ...
        if (memberIds.length > 0) {
          const reqData: LeaveRequest[] = await apiClient.get('/leave-requests/scope', {
        ...
        setError(err.message || langPackLabel("txtFailedToLoad") || 'Failed to load calendar data');
      } finally {
        setLoading(false);
      }
    };
          ...
          <>
            ...
            <DialogContent dividers>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                ...
                {selectedLeaveRequest.reason && <Box><Typography variant="caption" color="text.secondary">{langPackLabel("txtReason") || "Reason"}</Typography><Typography variant="body2">{selectedLeaveRequest.reason}</Typography></Box>}
              </Box>
            </DialogContent>
          </>
        )}
          ...
          <>
            ...
            <DialogContent dividers>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                ...
                {selectedHoliday.description && <Box><Typography variant="caption" color="text.secondary">{langPackLabel("txtDescription") || "Description"}</Typography><Typography variant="body2">{selectedHoliday.description}</Typography></Box>}
                {selectedHoliday.is_recurring && <Chip label="Recurring" size="small" color="info" sx={{ width: 'fit-content' }} />}
              </Box>
            </DialogContent>
          </>
        )}
        <DialogActions><Button onClick={() => setSelectedEvent(null)}>{langPackLabel("txtClose") || "Close"}</Button></DialogActions>
      </Dialog>
    </Box>
  );
};
