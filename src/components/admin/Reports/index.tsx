    ...
    <Box>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 3 }}>
          {error}
          <Button onClick={fetchReportData} sx={{ ml: 2 }}>{langPackLabel("txtRetry") || "Retry"}</Button>
        </Alert>
      )}
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            <InputLabel>{langPackLabel("txtCompany") || "Company"}</InputLabel>
            <Select
              value={selectedCompanyId}
              label={langPackLabel("txtCompany") || "Company"}
              onChange={(e) => handleCompanyChange(e.target.value)}
            >
              <MenuItem value="">{langPackLabel("txtAllCompanies") || "All Companies"}</MenuItem>
              {companies.map((c) => (
          ...
          <FormControl size="small" sx={{ minWidth: 130 }}>
            ...
            <Select
              value={statusFilter}
              label={langPackLabel("txtStatus") || "Status"}
              onChange={(e) => { setStatusFilter(e.target.value); setPage(0); }}
            >
              <MenuItem value="">{langPackLabel("txtAllStatuses") || "All Statuses"}</MenuItem>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 4 }}>
        ...
        <Card sx={{ flex: '1 1 180px', minWidth: '180px' }}>
          <CardContent>
            <Typography variant="body2" color="text.secondary">{langPackLabel("txtCompanies") || "Companies"}</Typography>
      ...
      {requests.length === 0 ? (
        <Alert severity="info">{langPackLabel("txtNoLeaveDataFound") || "No leave data found for the selected filters."}</Alert>
      ) : (
        <>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 4 }}>
            <Paper sx={{ p: 3, flex: '1 1 400px' }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                {langPackLabel("txtLeaveUsageByCompany") || "Leave Usage by Company"}
              </Typography>
            ...
            <Paper sx={{ p: 3, flex: '1 1 400px' }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                {langPackLabel("txtLeaveUsageByGroup") || "Leave Usage by Group"}
              </Typography>
              <ResponsiveContainer width="100%" height={300}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 4 }}>
            <Paper sx={{ p: 3, flex: '1 1 400px' }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                {langPackLabel("txtLeaveUsageByDepartment") || "Leave Usage by Department"}
              </Typography>
              <ResponsiveContainer width="100%" height={300}>
            ...
            <Paper sx={{ p: 3, flex: '1 1 400px' }}>
              <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
                {langPackLabel("txtLeaveUsageByType") || "Leave Usage by Type"}
              </Typography>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
          ...
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
              {langPackLabel("txtLeaveRequests") || "Leave Requests"}
            </Typography>
            <TableContainer>
              <Table size="small">
                <TableHead>
