...
import { Company, Group, Department, Team, HierarchyProfile, DEFAULT_WORKDAYS, DEFAULT_LEGAL_WORKDAYS } from '../../types';
...
const organizationSlice = createSlice({
  ...
  reducers: {
    ...
    setLegalWorkdays: (state, action) => {
      state.legalWorkdays = action.payload;
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchHierarchyProfile.fulfilled, (state, action) => {
        ...
        state.legalWorkdays = action.payload.legal_workdays ?? null;
      })
...
export const { clearOrganizationError, resetOrganizationState, setWorkdayConfig, setLegalWorkdays } = organizationSlice.actions;

export const selectWorkdayConfig = (state: RootState): number[] =>
  state.organization.workdayConfig ?? DEFAULT_WORKDAYS;

export const selectLegalWorkdays = (state: RootState): number[] =>
  state.organization.legalWorkdays ?? DEFAULT_LEGAL_WORKDAYS;
